...
class LihatLaporanKegiatanTests(TestCase):
    ...
    def test_unauthorized_access(self):
        ...
        response = self.client.get(reverse('lihat_laporan_kegiatan:lihat_laporan_kegiatan', args=['kp', self.mahasiswa.id]))
    ...
    def test_invalid_program(self):
        ...
        response = self.client.get(reverse('lihat_laporan_kegiatan:lihat_laporan_kegiatan', args=['invalid_program', self.mahasiswa.id]))
    ...
    def test_mahasiswa_not_found(self):
        ...
        self.client.login(username='penyelia_user', password='testpassword')
        response = self.client.get(reverse('lihat_laporan_kegiatan:lihat_laporan_kegiatan', args=['kp', 9999]))
        self.assertEqual(response.status_code, 404)
    ...
    def test_authorized_access_mbkm(self):
        ...
        self.client.login(username='penyelia_user', password='testpassword')
        response = self.client.get(reverse('lihat_laporan_kegiatan:lihat_laporan_kegiatan', args=['mbkm', self.mahasiswa.id]))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, self.mahasiswa.nama)
    ...
    #     self.assertEqual(response.status_code, 200)
    #     self.assertContains(response, self.penyelia.nama)
    #     self.assertContains(response, self.mahasiswa.nama)
