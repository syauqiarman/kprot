...
import testapp.validators as v
...
BELUM_SESUAI_FORMAT = "Sudah Dinilai Namun Belum Sesuai Format"
...
STATUS_LAPORAN_KP = [
    ...
    (BELUM_SESUAI_FORMAT, BELUM_SESUAI_FORMAT),
...
class Manajemen(MultiRolesUser):
    pass
...
class PendaftaranMBKM(models.Model):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.pk:
            self.history = [make_aware(datetime.now()).isoformat()]
            if not self.status_pendaftaran:
...
class LaporanKP(Laporan):
    ...
    def clean(self):
        super().clean()
        v.determine_status_and_nilai_laporan_kp(self)
...
class LaporanMBKM(Laporan):
    ...
    status = models.CharField(
        max_length=50,
        choices=STATUS_LAPORAN_MBKM,
        default=BELUM_DICEK,
    )
    sks_klaim = models.IntegerField()
    status_persetujuan_dosen = models.CharField(
        max_length=50, 
        choices=PERSETUJUAN_CHOICES, 
        default="-"
    )
...
def validate_one_role_user(user):
    user_has_any_role = (
        ...
        Manajemen.objects.filter(user=user).exists()
    )
